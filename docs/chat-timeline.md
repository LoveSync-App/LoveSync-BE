...
Gọi `POST /chat/location` để tạo item có `type: "LOCATION"`:
...
Đây là vị trí cố định trong lịch sử chat. Chia sẻ vị trí trực tiếp sử dụng API
và namespace `/locations` riêng, không cập nhật item snapshot này.
